...
describe("Testing Random number Generation and Perform Reveal after Public Sale...", function () {
    ...
    it("Should revert update uri function if it's not called by non-owner", async function () {
        let tx = sale.connect(user1).updateURI("http://example5.com");
    ...
    it("Should revert if public sale is not ended", async function () {
        ...
        //buying some tokens in Public Sale
        let _publicSaleMintCost = (await sale.publicSaleMintCost()).toNumber();

        await sale.connect(user1).publicSaleBuy(100, { value: 100 * _publicSaleMintCost });

        let _totalMint = await sale.totalMint();
    ...
    it("Should revert getAssetId before reveal", async function () {
        ...
        await expect(tx).to.be.revertedWith("reveal token first");
    });
